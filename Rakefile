...
task :check_homework do
  ...
  homework_count = Dir.entries('.').reject{|d| d.scan(/tmp.*/).empty?}.count
  homework_count.times do |counter|
    begin
      puts '=' * 80
      puts "tmp#{counter}"
      path = "./tmp#{counter}/sandbox#{counter}/ruby_bursa_task_2/#{file_name}.rb"
      text = File.read(path)
